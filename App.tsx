...
const App = () => {
  ...
  const showLocalNotification = useCallback((msg: BroadcastMessage) => {
    if (!notificationsEnabled || Notification.permission !== 'granted') return;
    
    const btn = buttons.find(b => b.id === msg.buttonId);
    const label = btn?.label || 'Action';
    
    new Notification(`StreamCast: ${label}`, {
        ...
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex gap-2">
            <Button 
              variant="ghost" 
              size="icon" 
              className={cn("rounded-full", notificationsEnabled ? "text-primary" : "text-muted-foreground")}
              onClick={() => notificationPermission === 'granted' ? setNotificationsEnabled(!notificationsEnabled) : requestNotificationPermission()}
            >
              <Icon name={notificationsEnabled ? "bell" : "bell-off"} className="w-5 h-5" />
            </Button>
          ...
          <Card className="bg-black/40 border-border overflow-hidden">
            <CardContent className="p-0 h-[400px] overflow-y-auto font-mono text-[11px]">
              ...
              <div className="divide-y divide-border">
                {logs.map((log, i) => (
                  <div key={i} className="p-3 bg-white/[0.01] hover:bg-white/[0.03] animate-in fade-in slide-in-from-left-2">
                    <div className="flex justify-between items-center mb-1">
                      <span className="font-bold text-primary">{buttons.find(b => b.id === log.buttonId)?.label || log.buttonId}</span>
